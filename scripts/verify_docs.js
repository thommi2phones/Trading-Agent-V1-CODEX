 ...
 *     docs/verification_and_taxonomy_runbook.md.
 *   - Every repo-relative FILE reference in backticks
 *     (lib/*.js, webhook/*.js, scripts/*.js, docs/*.md, …) resolves.
 *
 * Exclusions:
 *   - Placeholders like `<role>`, `<envelope_id>`.
 *   - Wildcard globs like `scripts/verify_*.js`.
 *   - Markdown link text where the target is an http URL.
 *   - Files under `webhook/data/` or `data/macro_snapshots/` (runtime).
 */
...
function listVerifyScripts() {
  return fs.readdirSync(scriptsDir).filter((f) => /^verify_.*\.js$/.test(f)).sort();
}

function readAllDocs() {
  return fs.readdirSync(docsDir).filter((f) => f.endsWith(".md")).map((f) => {
...
const PATH_PREFIXES = ["lib/", "scripts/", "docs/", "tv_direct/", "webhook/", "data/", "tests/", "integration/"];
...
for (const { body } of docs) {
  for (const m of stripExternalLinks(body).matchAll(/\bverify_[a-z_]+\.js\b/g)) verifyRefs.add(m[0]);
}
